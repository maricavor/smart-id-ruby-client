...
module SmartId
  ...
  class DeviceLinkBuilder
    ...
    # Java-style aliases for migration compatibility.
    def withSchemeName(value)
      with_scheme_name(value)
    end

    def withDeviceLinkBase(value)
      with_device_link_base(value)
    end
    ...
    def withDeviceLinkType(value)
      with_device_link_type(value)
    end

    def withSessionType(value)
      with_session_type(value)
    end

    def withSessionToken(value)
      with_session_token(value)
    end

    def withElapsedSeconds(value)
      with_elapsed_seconds(value)
    end
    ...
    def withDigest(value)
      with_digest(value)
    end

    def withRelyingPartyName(value)
      with_relying_party_name(value)
    end

    def withBrokeredRpName(value)
      with_brokered_rp_name(value)
    end

    def withInteractions(value)
      with_interactions(value)
    end

    def withInitialCallbackUrl(value)
      with_initial_callback_url(value)
    end

    def createUnprotectedUri
      create_unprotected_uri
    end

    def buildDeviceLink(session_secret)
    ...
    def generate_auth_code(unprotected_link, session_secret)
      raise_request_setup_error("Parameter 'sessionSecret' cannot be empty") if blank?(session_secret)
      ...
      key = Base64.strict_decode64(session_secret)
      ...
      raise SmartId::Errors::RequestSetupError, "Failed to calculate authCode: #{e.message}"
    end
    ...
    def raise_request_setup_error(message)
      raise SmartId::Errors::RequestSetupError, message
    end
  end
end
