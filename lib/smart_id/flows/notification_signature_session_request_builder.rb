    ...
    class NotificationSignatureSessionRequestBuilder < BaseBuilder
      ...
      def with_signable_data(signable_data)
        if digest_input_kind == :signable_hash
          raise SmartId::Errors::RequestSetupError, "Value for 'digestInput' has already been set with SignableHash"
        end
      ...
      def with_signable_hash(signable_hash)
        if digest_input_kind == :signable_data
          raise SmartId::Errors::RequestSetupError, "Value for 'digestInput' has already been set with SignableData"
        end
      ...
      def validate_request_parameters
        ...
        if @digest_input.nil?
          raise SmartId::Errors::RequestSetupError, "Value for 'digestInput' must be set with either SignableData or SignableHash"
        end
      ...
      def validate_nonce
        ...
        raise SmartId::Errors::RequestSetupError, "Value for 'nonce' length must be between 1 and 30 characters"
      end
      ...
      def validate_response_parameters(response)
        ...
        if verification_code.nil?
          raise SmartId::Errors::UnprocessableResponseError, "Notification-based signature response field 'vc' is missing"
        end
        ...
        unless VERIFICATION_CODE_PATTERN.match?(vc_value.to_s)
          raise SmartId::Errors::UnprocessableResponseError,
