  ...
  module Flows
    ...
    class CertificateByDocumentNumberRequestBuilder < BaseBuilder
      ...
      def validate_response_parameters(response)
        if response.nil?
          raise SmartId::Errors::UnprocessableResponseError, "Queried certificate response is not provided"
        end

        validate_state(response)
        cert = fetch_value(response, :cert)
        if cert.nil?
          raise SmartId::Errors::UnprocessableResponseError, "Queried certificate response field 'cert' is missing"
        end
      ...
      def validate_state(response)
        ...
        if blank?(state)
          raise SmartId::Errors::UnprocessableResponseError, "Queried certificate response field 'state' is missing"
        end
        unless SUPPORTED_STATES.include?(state)
          raise SmartId::Errors::UnprocessableResponseError, "Queried certificate response field 'state' has unsupported value"
        end
      ...
      def validate_certificate_level(cert)
        ...
        if blank?(response_level)
          raise SmartId::Errors::UnprocessableResponseError, "Queried certificate response field 'cert.certificateLevel' is missing"
        end
        ...
        if CERTIFICATE_LEVEL_ORDER[response_level] < CERTIFICATE_LEVEL_ORDER.fetch(requested_level, CERTIFICATE_LEVEL_ORDER[DEFAULT_CERTIFICATE_LEVEL])
          raise SmartId::Errors::UnprocessableResponseError, "Queried certificate has lower level than requested"
        end
      end
      ...
      def validate_certificate_value(cert)
        ...
        if blank?(cert_value)
          raise SmartId::Errors::UnprocessableResponseError, "Queried certificate response field 'cert.value' is missing"
        end
        unless BASE64_PATTERN.match?(cert_value)
          raise SmartId::Errors::UnprocessableResponseError,
