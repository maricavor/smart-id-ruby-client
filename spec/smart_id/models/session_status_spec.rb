...
RSpec.describe SmartId::Models::SessionStatus do
  it "maps nested session status payload to typed models" do
    ...
    expect(model.result).to be_a(SmartId::Models::SessionResult)
    ...
    expect(model.signature).to be_a(SmartId::Models::SessionSignature)
    expect(model.signature.signature_algorithm_parameters.hash_algorithm).to eq("SHA-256")
    expect(model.cert).to be_a(SmartId::Models::SessionCertificate)
